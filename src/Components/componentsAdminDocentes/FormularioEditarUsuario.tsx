...
const FormularioEditarUsuario: React.FC<FormularioEditarUsuarioProps> = ({ userId, onClose, onSave }) => {
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<Partial<User>>(); // Utilizando useForm
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        setUser(userData);
        setValue('nombre', userData.nombre); // Establecer los valores iniciales
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-lg w-[600px]"> {/* Ajuste de ancho a 600px */}
        <h2 className="text-lg font-bold mb-4">Editar Usuario</h2>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-2 gap-4"> {/* Estructura en columnas */}
            <div className="mb-4">
